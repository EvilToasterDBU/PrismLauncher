...
#define HEX_TO_IMVEC4(hex, alpha) \
	...
	// BigScreen note: PCSX2 carries a local patch to stock Dear ImGui adding
	// ImFont::LineHeight (see their 3rdparty/imgui/CHANGELOG.txt, referencing
	// https://github.com/ocornut/imgui/issues/9063), which we don't vendor.
	// An earlier version of this shim used LegacySize as a stand-in, on the
	// theory it was "the size for old code expecting one on ImFont" — wrong:
	// LegacySize is a pixel size, same as font.second, so multiplying them
	// (as PCSX2's ratio-based LineHeight patch intends) computed pixels²,
	// not pixels — e.g. two ~22px quantities multiplying to ~484px, wildly
	// inflating every menu row's height (found via a screenshot showing a
	// single toggle occupying most of the screen). The doc comment above is
	// the authoritative definition — just ask CalcTextSizeA directly.
	static __fi float GetLineHeight(std::pair<ImFont*, float> font)
	{
		return font.first->CalcTextSizeA(font.second, FLT_MAX, 0.0f, "A").y;
	}
