...
// BigScreen note: plutovg/plutosvg (SVG rasterization) aren't vendored yet —
// see the BigScreen plan, milestone M1. LoadSvgTextureImage() below is
// stubbed to fail gracefully (callers already fall back to the placeholder
// texture) rather than pulling in another third-party library for
// something the M1 nav demo doesn't need. Revisit once real SVG icon/cover
// assets are wired up.
...
{
	// Stubbed for v1 (see the file-level note above these functions): SVG
	// rasterization isn't wired up yet, so callers fall back to the
	// placeholder texture, same as any other failed-to-load image.
	(void)size;
	(void)mode;
	Console.Error("SVG loading not yet implemented in BigScreen, skipping '%s'", path);
	return std::nullopt;
}
...
void ImGuiFullscreen::DrawNotifications(ImVec2& position, float spacing)
{
	...
	for (u32 index = 0; index < static_cast<u32>(s_notifications.size());)
	{
		...
		if (has_badge)
		{
			...
			if (tex)
			{
				dl->AddImage(static_cast<ImTextureID>(tex->GetNativeHandle()), badge_min, badge_max, ImVec2(0.0f, 0.0f), ImVec2(1.0f, 1.0f),
